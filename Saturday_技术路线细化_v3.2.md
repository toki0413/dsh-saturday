...
本版在 v3.1 评审基础上修订，DSH 作为产品既定底座，修订重点是：**把对底座的依赖从"隐性押注"变为"显性治理"，把过满的排期改为有资源支撑的计划，修正若干概念性技术缺陷**。
...
- **明确不做自动耦合**：MD→相场的物理映射（位错、损伤变量传递）是研究问题，由用户在领域软件中完成，Saturday 只保证数据正确、格式正确、可溯源
