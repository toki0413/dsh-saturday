...
test('3. parseMilestoneTable（真实契约文档）：63 条且摘要层条款可追溯', () => {
  const md = readFileSync(join(repoRoot, 'packages', 'bridge', 'docs', 'plugin-contract-v0.md'), 'utf8')
  const rows = parseMilestoneTable(md)
  assert.equal(rows.length, 63, '附录 A 当前应为 63 条实证映射')
  assert.equal(rows[rows.length - 1].index, 63)
  assert.ok(rows[35].evidence.includes('scripts/summary'), '第 36 条（摘要层自身）证据必须可追溯到摘要层测试')
  assert.ok(rows[rows.length - 1].evidence.includes('plugin-mixture-quota') && rows[rows.length - 1].evidence.includes('demo:agent'),
    '第 63 条证据指向锚点工具 Agent 层暴露 + 配额闭式对账（⑯/⑰）')
  assert.ok(rows[rows.length - 2].evidence.includes('anchor-autoingest'), '第 62 条证据指向闭环轨迹自动入库测试（⑮）')
  assert.ok(rows[rows.length - 3].evidence.includes('demo:anchor-guided') && rows[rows.length - 3].evidence.includes('plugin-anchor-tools'),
    '第 61 条证据指向锚点引导闭环端到端 + 工具链契约审查（⑫/⑭）')
  assert.ok(rows[rows.length - 4].evidence.includes('plugin-anchor-tools'), '第 60 条证据指向锚点引导混合提案工具层测试（⑩）')
  assert.ok(rows[rows.length - 5].evidence.includes('anchor-store'), '第 59 条证据指向锚点库测试（⑦）')
  assert.ok(rows[rows.length - 6].evidence.includes('availability'), '第 58 条证据指向可用性预检工具测试（⑥）')
  assert.ok(rows[rows.length - 7].evidence.includes('evidence'), '第 57 条证据指向证据源独立性机器审计测试（⑤）')
  assert.ok(rows[rows.length - 8].evidence.includes('plugin-screening'), '第 56 条证据指向混合熵第二内置源测试')
  assert.ok(rows[rows.length - 9].evidence.includes('demo-mixture-sampling'), '第 55 条证据指向多锚点混合采样真实演示')
  assert.ok(rows[rows.length - 10].evidence.includes('demo-availability'), '第 54 条证据指向可用性预检演示')
  assert.ok(rows[rows.length - 11].evidence.includes('units.test') && rows[rows.length - 11].evidence.includes('potential.test'),
    '第 53 条证据指向指纹实测态回读测试（通配三态 + 盖章三态）')
  assert.ok(rows[rows.length - 12].evidence.includes('plugin-screening'), '第 52 条证据指向换算审计通道测试')
  assert.ok(rows[rows.length - 13].evidence.includes('plugin-screening'), '第 51 条证据指向工具层参考态声明形态测试')
  assert.ok(rows[rows.length - 14].evidence.includes('demo-cross-engine'), '第 50 条证据指向跨引擎对照演示')
  assert.ok(rows[rows.length - 15].evidence.includes('potential.test'), '第 49 条证据指向 M2 激活门禁测试')
  assert.ok(rows[rows.length - 16].evidence.includes('units.test') && rows[rows.length - 16].evidence.includes('plugin-screening'),
    '第 48 条证据指向单位/指纹门禁测试（core 纯层 + 筛选层 M3）')
  assert.ok(rows[rows.length - 17].evidence.includes('plugin-sampler-ou'), '第 47 条证据指向多锚点混合采样测试')
  assert.ok(rows[rows.length - 18].evidence.includes('plugin-sampler-ou'), '第 46 条证据指向采样温度标定测试')
  assert.ok(rows[rows.length - 19].evidence.includes('plugin-screening'), '第 45 条证据指向证据源注册表化测试')
