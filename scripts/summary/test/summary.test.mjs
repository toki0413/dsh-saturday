...
test('3. parseMilestoneTable（真实契约文档）：36 条且末条指向摘要层自身', () => {
  const md = readFileSync(join(repoRoot, 'packages', 'bridge', 'docs', 'plugin-contract-v0.md'), 'utf8')
  const rows = parseMilestoneTable(md)
  assert.equal(rows.length, 36, '附录 A 当前应为 36 条实证映射')
  assert.equal(rows[rows.length - 1].index, 36)
  assert.ok(rows[rows.length - 1].evidence.includes('scripts/summary'), '第 36 条证据必须可追溯到摘要层测试')
