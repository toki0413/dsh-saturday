// 发布形态核验：逐包 `npm pack --dry-run` 干跑，机械核验——
//   一者 版本一致性：全部包版本与根包一致（发布物不夹带漂移版本）；
//   二者 files 白名单生效：发布物不含测试/日志/临时产物（只含实现与必要数据面）；
//   三者 发布物清单如实呈报（包名/条目数/解压体积随报告呈现，可人工复核）。
// 本脚本只做核验不是发布：全程 --dry-run，不产生 .tgz 不触网。违规即非零退出（白名单是门禁不是建议）。
...
console.log(`══ 发布形态核验：${report.length} 包干跑，根包版本 ${root.version} ══`)
...
console.log('══ 核验通过：版本一致 + 白名单无夹带（本脚本不执行发布，未产生任何 .tgz）══')
