...
等 56 个工具都在。想要真实 EMT/LAMMPS 物理，再装 Python≥3.10 + ASE（见下“环境矩阵”），
...
上述工具面经 `@toki0413/mcp-server` 以 Model Context Protocol 全量暴露（56 工具，
stdio 传输；工具面随环境如实收缩——无 `MP_API_KEY` 时 `structure.resolve` 不注册 = 55 工具，
