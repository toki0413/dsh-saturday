...
上游变化沿谱系自动传播失效与重算。推导登记簿（`@saturday/plugin-derivation`）
...
| 活性上下文 | `derivation.*` | 推导登记簿：导出量声明推导来源，失效沿推导图向下游传播，冻结结果只追加修正不重算（`@saturday/plugin-derivation`） |
...
packages/
  ...
  contract-tests/             # @saturday/contract-tests —— 契约测试套件（兼容性由测试承诺）
...
plugins/                      # 插件生态（新插件必须过 contract-tests 套件）
  screening/                  #   @saturday/plugin-screening —— 工作流：批量掺杂筛选
  mp-structure-source/        #   @saturday/plugin-mp —— 结构源：Materials Project
  ...
  derivation/                 #   @saturday/plugin-derivation —— 活性上下文：失效传播与惰性重算
```
...
npm run demo:anchor-guided --workspace @saturday/bridge      # 锚点引导闭环（入库→检索→提案→回算→排序）
npm run demo:anchor-auto --workspace @saturday/bridge        # 全自动锚点闭环（弛豫产物自动入库）
npm run demo:anchor-resume --workspace @saturday/bridge      # 跨会话恢复（落盘→回填→续供，谱系不断）
npm run demo:agent --workspace @saturday/bridge              # Agent 会话端到端（mock LLM，十个阶段）
```
...
- **契约即宪法**：`@saturday/contract-tests` 提供 structure-resolver / potential-provider /
