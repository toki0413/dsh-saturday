...
The whole tool surface (52 tools) is exposed over the Model Context Protocol via
`@toki0413/mcp-server` (stdio; the surface shrinks honestly with the environment — without
`MP_API_KEY`, `structure.resolve` is not registered = 51 tools; engines/structure sources with
