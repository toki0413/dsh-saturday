...
The whole tool surface (38 tools) is exposed over the Model Context Protocol via
`@toki0413/mcp-server` (stdio; the surface shrinks honestly with the environment — without
`MP_API_KEY`, `structure.resolve` is not registered = 37 tools; engines/structure sources with
