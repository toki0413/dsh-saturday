...
`potential.relax` and returns `energyPerAtom` (eV); all 55 tools (`material.load`,
...
The whole tool surface (55 tools) is exposed over the Model Context Protocol via
`@toki0413/mcp-server` (stdio; the surface shrinks honestly with the environment — without
`MP_API_KEY`, `structure.resolve` is not registered = 54 tools; engines/structure sources with
...
included); tool failures surface as MCP `isError` responses carrying structured error codes.
