// 高斯过程回归 + 贝叶斯优化（1D 标量 / 向量特征输入）——@toki0413/core 域无关纯数值，零依赖。
// 由 plugin-explore 的贝叶斯优化工作流与 plugin-screening 的 GP 能量证据源共用（单一实现）。
//
// 定位（诚实）：GP 是有噪声观测下的后验均值/方差模型，用作昂贵黑箱目标 E(x) 的代理；
//   采集函数 LCB(x)=mu−kappa·sigma（极小化版 UCB）指导下一评估点。随机优化启发式：不承诺
//   收敛到全局最优、不承诺有限步到 ε-邻域；对平滑单峰目标经验上少评估逼近极小。
//   数值全闭式可对账（Cholesky 解 SPD、GP 插值性质、合成目标 BO 收敛）。
//
// 纪律：纯函数、无外部依赖（自带 Cholesky 与回代）；病态（非正定/退化核）显式抛错，不静默加
//   大抖动掩盖。输入 x 可为标量或等长数值向量（向量走各向同性 RBF，单一 lengthScale，非 ARD）。

function gpError(code, msg) { const e = new Error(`${msg} (${code})`); e.code = code; return e }

/** RBF（平方指数）核：k(a,b)=sf²·exp(−‖a−b‖²/(2·ls²))；a,b 支持标量或等长数值向量 */
export function rbf(a, b, { ls = 1.0, sf = 1.0 } = {}) {
  let d2
  if (Array.isArray(a) || Array.isArray(b)) {
    if (!Array.isArray(a) || !Array.isArray(b) || a.length !== b.length) {
      throw gpError('GP_BAD_INPUT', 'vector kernel requires both inputs as equal-length arrays')
    }
    d2 = 0
  ...
  return sf * sf * Math.exp(-0.5 * d2 / (ls * ls))
}
...
function transpose(L) { return L[0].map((_, c) => L.map(r => r[c])) }

/**
 * GP 训练：给定观测 (xs, ys)（xs 元素为标量或等长向量），常数先验均值 + 高斯噪声（极小 jitter
 * 保数值稳定），预计算 K 的 Cholesky 与 alpha=K⁻¹(y−mean) 供后验闭式使用。
 */
export function gpTrain(xs, ys, { ls = 1.0, sf = 1.0, noise = 1e-6, mean = 0 } = {}) {
  ...
  const alpha = solveUpper(transpose(L), yv)
  return { xs, ys, mean, ls, sf, noise, alpha, L, n }
}

/** GP 后验：对查询 x（标量或向量）给 (mean, variance)。variance 数值下限夹到 ≥0。 */
export function gpPredict(model, x) {
  ...
  const v = solveUpper(transpose(model.L), yv)
  let kkv = 0
...
/**
 * 单目标贝叶斯优化（1D 标量域，极小化昂贵黑箱）。async objective(x)→number。
 * @returns {Promise<{ bestX, bestY, evaluations, history, note }>}
 */
...
/** 2D 超体积（最小化，ref 支配所有点）：按 obj0 升序扫掠，取运行 obj1 下降的前沿。闭式可验证。 */
