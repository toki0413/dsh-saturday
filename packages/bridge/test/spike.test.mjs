...
test('8. 卸载回退：dispose 后服务与工具全部回收（effect 语义）', async () => {
  ...
  try {
    ...
    assert.equal(h2.rt.tools.list().length, 8, '核心插件工具基线：material.load / structure.fromSmiles / potential.relax / engine.availability + 运行时动词面四件（capability.list / engine.attach / engine.detach / engine.crossCheck；workflow.screen 已迁出为独立插件）')
  } finally {
    await f2.dispose()
  }
