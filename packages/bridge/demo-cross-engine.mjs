...
// 环境自适应：基础引擎随数据面形态（有 Python 为 emt-mock；纯 Node 为 lj-js），
// 对照引擎为确定性 dft-mock 桩（纯 JS），两环境均完整可跑。
// 运行：node demo-cross-engine.mjs

import { Context } from '@deepseek-ai/cordis'
import plugin from './src/saturday.plugin.mjs'
import screeningPlugin, { screenDopants } from '@toki0413/plugin-screening'
...
console.log(`── A. 筛选交付自带能量来源可追溯性（${dataPlane} 引擎）──`)
...
// ── B. M3 拦截：DFT 指纹的参考态混进数据面引擎的凸包 → 显式拒绝 ──
...
const d1 = await screenDopants({ material: cu, dopants: ['Ag'], potential, engine: dataPlane })
...
console.log(`\n诚实声明：两份能量来自不同 software/method 指纹（${dataPlane} vs dft-mock 桩），`)
