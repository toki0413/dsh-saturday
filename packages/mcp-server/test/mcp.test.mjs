...
test('1. 全量插件挂载：37 工具、无重名、seam 前缀齐全', async () => {
  assert.equal(PLUGIN_MANIFEST.length, 20, 'bridge + 19 插件')
  await withServer(async (client) => {
    ...
    for (const expected of ['material.load', 'structure.fromSmiles', 'potential.relax', 'analysis.phonon', 'analysis.phonon.thermo', 'analysis.elasticity', 'sampler.flow', 'sampler.rss', 'workflow.screen', 'derivation.record', 'runtime.capability.list', 'runtime.engine.attach', 'runtime.engine.detach']) {
