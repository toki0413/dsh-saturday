...
test('1. 全量插件挂载：32 工具、无重名、seam 前缀齐全', async () => {
  assert.equal(PLUGIN_MANIFEST.length, 19, 'bridge + 18 插件')
  await withServer(async (client) => {
    ...
    for (const expected of ['material.load', 'structure.fromSmiles', 'potential.relax', 'analysis.phonon', 'sampler.flow', 'sampler.rss', 'workflow.screen', 'derivation.record']) {
